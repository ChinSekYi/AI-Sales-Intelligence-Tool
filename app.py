...
from src.get_news import (deduplicate_articles, fetch_news_by_query,
                          save_news_to_file)
            ...
            if mode == "Sales Triggers":
                ...
                if unique_articles:
                    ...
                    query_params = {
                        "mode": "Sales Triggers",
                        "days_back": days_back,
                        "region": region if region != "None (Global)" else None,
                        "sort_by": sort_by,
                        "triggers": [name for name, _ in custom_queries],
                    }
                    save_news_to_file(
                        news_data,
                        filename="sales_triggers.json",
                ...
                if news_data and news_data.get("status") == "ok":
                    ...
                    save_news_to_file(news_data, query_params=query_params)
...
if news_data is None and os.path.exists(data_file):
    try:
        ...
        if "metadata" in loaded_data:
            ...
            if is_expired:
                st.warning(
                    f"⚠️ Cached data is {age_hours:.1f} hours old (expired). Click 'Fetch Latest News' to refresh."
                )
            elif age_hours > 12:
                st.info(
                    f"ℹ️ Showing cached data from {age_hours:.1f} hours ago. Consider refreshing for latest updates."
                )

            # Show when data was fetched in sidebar
            with st.sidebar:
                st.caption(f"📅 Data fetched: {fetched_at.strftime('%Y-%m-%d %H:%M')}")
                st.caption(f"⏰ Cache age: {age_hours:.1f} hours")

            news_data = loaded_data
        else:
            # Old format without metadata - show warning
            st.warning(
                "⚠️ Cached data has no timestamp. Age unknown. Consider fetching fresh data."
            )
            news_data = loaded_data
